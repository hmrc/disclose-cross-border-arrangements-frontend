  ...
  "ValidationEngine" - {
    "ValidateXml" - {
      ...
      "must return ValidationFailure for valid file which fails metaDataCheck and audit outcome" in new SetUp {
        ...
        when(mockMetaDataValidationService.verifyMetaData(any(), any())(any(), any()))
          .thenReturn(Future.successful(Seq(Validation("metaDataRules.arrangementId.arrangementIdDoesNotMatchRecords", false))))
      ...
      "must return ValidationFailure for file which multiple pieces of mandatory information missing" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(addressError1, addressError2, cityError1, cityError2)))
        when(mockMetaDataValidationService.verifyMetaData(any(), any())(any(), any())).thenReturn(Future.successful(Seq()))
      ...
      "must return ValidationFailure for file missing mandatory attributes" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(missingAttributeError)))
      ...
      "must return ValidationFailure for file where element is too long (1-400 allowed)" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(maxLengthError1, maxlengthError2)))
      ...
      "must return ValidationFailure for file where element is too long (1-4000 allowed)" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(maxLengthError3, maxlengthError4)))
      ...
      "must return ValidationFailure for file with invalid country code" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(countryCodeError1, countryCodeError2)))
      ...
      "must return ValidationFailure for file with invalid countryMS code" in new SetUp {
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(concernedMsError1, concernedMsError2)))
      ...
      "must return ValidationFailure for file with invalid countryExemption code" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(countryExemptionError1, countryExemptionError2)))
      ...
      "must return ValidationFailure for file with invalid Reason entry code" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(reasonError1, reasonError2)))
      ...
      "must return ValidationFailure for file with invalid Intermediary Capacity code" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(intermediaryCapacityError1, intermediaryCapacityError2)))
      ...
      "must return ValidationFailure for file with invalid RelevantTaxpayer Discloser Capacity code" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(relevantTpDiscloserCapacityError1, relevantTpDiscloserCapacityError2)))
      ...
      "must return ValidationFailure for file with invalid issuedBy code" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(issuedByError1, issuedByError2)))
      ...
      "must return ValidationFailure with generic error message if parse error is not in an expected format" in new SetUp {
        ...
        when(mockXmlValidationService.validateXml(any())).thenReturn((elem, ListBuffer(randomParseError)))
      ...
      "must return a ValidationFailure with only xmlErrors if Business Rules check is not required" in new SetUp {
        ...
        Await.result(validationEngine.validateFile(source, enrolmentId, businessRulesCheckRequired = false), 10 seconds) mustBe Right(
          ValidationFailure(expectedErrors)
        )
        verify(mockAuditService, times(1)).auditValidationFailure(any(), any(), any())(any())
        verify(mockAuditService, times(1)).auditErrorMessage(any())(any())
