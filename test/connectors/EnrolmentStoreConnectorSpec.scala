/*
 * Copyright 2020 HM Revenue & Customs
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package connectors

import base.SpecBase
import config.FrontendAppConfig
import models.enrolments.{Enrolment, EnrolmentResponse, KnownFact}
import org.mockito.Matchers.any
import org.mockito.Mockito.when
import play.api.inject.bind
import play.api.libs.json.{JsValue, Json}
import uk.gov.hmrc.http.{HttpClient, HttpResponse}
import utils.EnrolmentConstants

import scala.concurrent.Future

class EnrolmentStoreConnectorSpec extends SpecBase {

  val mockHttpClient = mock[HttpClient]

  val application = applicationBuilder(Some(emptyUserAnswers))
    .overrides(
      bind[FrontendAppConfig].toInstance(mockAppConfig),
      bind[HttpClient].toInstance(mockHttpClient)
    ).build()

  val enrolmentConnector = application.injector.instanceOf[EnrolmentStoreConnector]

  val enrolmentResponse = """
                            |{
                            |    "service":"HMRC-DAC6-ORG",
                            |    "enrolments":[
                            |
                            |                {
                            |                    "identifiers": [
                            |
                            |                            {
                            |                                "key":"DAC6ID",
                            |                                "value":"id"
                            |                            }
                            |
                            |                    ],
                            |                    "verifiers": [
                            |
                            |                            {
                            |                                "key":"CONTACTNAME",
                            |                                "value":"test testing"
                            |                            }
                            |                        ,
                            |                            {
                            |                                "key":"EMAIL",
                            |                                "value":"me@test.com"
                            |                            }
                            |
                            |                    ]
                            |                }
                            |
                            |    ]
                            |}
          """.stripMargin

  "getEnrolments" - {
    "must return correct identifiers" in {

      val enrolmentResponseJson = Json.parse(enrolmentResponse)

      when(mockHttpClient.POST[JsValue, HttpResponse](any(), any(), any())(any(), any(), any(), any()))
        .thenReturn(Future.successful(HttpResponse(200, enrolmentResponseJson, Map.empty[String,Seq[String]])))

      val identifiers = Seq(KnownFact(EnrolmentConstants.dac6IdentifierKey, "id"))
      val verifiers = Seq(
        KnownFact("CONTACTNAME", "test testing"),
        KnownFact("EMAIL", "me@test.com"))

      whenReady( enrolmentConnector.getEnrolments("id")){
        result =>
          result mustBe Some(EnrolmentResponse(EnrolmentConstants.dac6EnrolmentKey, Seq(Enrolment(identifiers, verifiers))))
      }
    }
    "return None for an invalid enrolmentID" in {
      val enrolmentResponseJson = Json.parse(enrolmentResponse)

      when(mockHttpClient.POST[JsValue, HttpResponse](any(), any(), any())(any(), any(), any(), any()))
        .thenReturn(Future.successful(HttpResponse(404, "")))

      val identifiers = Seq(KnownFact(EnrolmentConstants.dac6IdentifierKey, "wrong-id"))
      val verifiers = Seq(
        KnownFact("CONTACTNAME", "test testing"),
        KnownFact("EMAIL", "me@test.com"))

      whenReady( enrolmentConnector.getEnrolments("id")){
        result =>
          result mustBe None
      }
    }
    "return None for an invalid JSON" in {
      val enrolmentResponse = """
                                |{
                                |    "service":"HMRC-DAC6-ORG",
                                |    "ENROLMENTS-INVAL":[
                                |
                                |                {
                                |                    "identifiers": [
                                |
                                |                            {
                                |                                "key":"DAC6ID",
                                |                                "value":"id"
                                |                            }
                                |
                                |                    ],
                                |                    "verifiers": [
                                |
                                |                            {
                                |                                "key":"CONTACTNAME",
                                |                                "value":"test testing"
                                |                            }
                                |                        ,
                                |                            {
                                |                                "key":"EMAIL",
                                |                                "value":"me@test.com"
                                |                            }
                                |
                                |                    ]
                                |                }
                                |
                                |    ]
                                |}
          """.stripMargin
      val enrolmentResponseJson = Json.parse(enrolmentResponse)

      when(mockHttpClient.POST[JsValue, HttpResponse](any(), any(), any())(any(), any(), any(), any()))
        .thenReturn(Future.successful(HttpResponse(200, enrolmentResponseJson, Map.empty[String,Seq[String]])))

      val identifiers = Seq(KnownFact(EnrolmentConstants.dac6IdentifierKey, "wrong-id"))
      val verifiers = Seq(
        KnownFact("CONTACTNAME", "test testing"),
        KnownFact("EMAIL", "me@test.com"))

      whenReady( enrolmentConnector.getEnrolments("id")){
        result =>
          result mustBe None
      }
    }

  }

}
