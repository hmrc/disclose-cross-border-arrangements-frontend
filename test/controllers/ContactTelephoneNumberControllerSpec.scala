...
class ContactTelephoneNumberControllerSpec extends SpecBase with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  "ContactTelephoneNumber Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ContactTelephoneNumberPage, "0191 111 2222").success.value
      ...
      val filledForm = form.bind(Map("telephoneNumber" -> "0191 111 2222"))
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, contactTelephoneNumberRoute)
          .withFormUrlEncodedBody(("telephoneNumber", "0191 111 2222"))
