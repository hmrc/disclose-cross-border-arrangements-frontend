...
class CheckForSubmissionActionProvider @Inject() ()(implicit val executionContext: ExecutionContext) extends ActionRefiner[DataRequest, DataRequest] {

  override protected def refine[A](request: DataRequest[A]): Future[Either[Result, DataRequest[A]]] =
    if (request.userAnswers.get(JourneyInProgressPage).isEmpty) {
      Future.successful(Left(Redirect(routes.DisclosureAlreadySentController.onPageLoad())))
    } else {
